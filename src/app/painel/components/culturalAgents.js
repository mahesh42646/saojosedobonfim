...
function AgentDetails({ agent, onBack, user }) {
  ...
  const generatePDF = () => {
    ...
    doc.text(`Generated by: ${user?.name || 'Admin'} • Date: ${new Date().toLocaleDateString()}`, 15, 10);
    
    // Main title
    doc.setFontSize(18);
    doc.setTextColor(0, 0, 0);
    doc.setFont(undefined, 'bold');
    doc.text('Cultural Agent Details', 15, yPosition);
    yPosition += 15;
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(`Agent Type: ${getAgentType(agent)}`, 15, yPosition);
    ...
    addSection('Personal Information', [
      ...
      { label: 'Social Name', value: agent.socialname },
      { label: 'Gender', value: agent.gender },
      { label: 'Breed', value: agent.breed },
      { label: 'LGBTQIAPN+', value: agent.lgbtq },
      { label: 'Date of Birth', value: formatDateForPDF(agent.dob) },
      { label: 'RG', value: agent.rg }
    ]);
    
    // Accessibility Information
    addSection('Accessibility Information', [
      { label: 'Has PCD Disability', value: agent.pcd },
      { label: 'PCD Details', value: agent.pcd !== 'Não' ? agent.withoutPcd : null }
    ]);
    
    // Socioeconomic and Educational Information
    addSection('Socioeconomic & Educational Information', [
      { label: 'Education', value: agent.education },
      { label: 'Individual Income', value: agent.income },
      { label: 'Social Program Beneficiary', value: agent.socialProgramBeneficiary },
      { label: 'Social Program Name', value: agent.socialProgramName }
    ]);
    
    // Professional Information
    addSection('Professional Information', [
      { label: 'Main Area of Activity', value: agent.mainActivity },
      { label: 'Traditional Communities', value: agent.traditionalCommunities },
      { label: 'Other Activity', value: agent.otherActivity }
    ]);
    ...
    addSection('Address', [
      ...
      { label: 'Street and Number', value: agent.street }
    ]);
    
    // Contact
    addSection('Contact', [
      { label: 'Telephone', value: agent.telephone },
      { label: 'Email', value: agent.email }
    ]);
    
    // Responsible for Registration
    if (agent.responsible) {
      addSection('Responsible for Registration', [
        { label: 'Responsible Person', value: agent.responsible }
      ]);
    }
    ...
    if (agent.typeStatus?.business?.isComplete && agent.businessData) {
      addSection('Business Information', [
        { label: 'CNPJ Type', value: agent.businessData.cnpjType },
        { label: 'Razão Social', value: agent.businessData.razaoSocial },
        { label: 'Nome Fantasia', value: agent.businessData.nomeFantasia },
        { label: 'CNPJ Number', value: agent.businessData.cnpj }
      ]);
    }
    
    if (agent.typeStatus?.collective?.isComplete && agent.collectiveData) {
      addSection('Collective Information', [
        { label: 'Collective Name', value: agent.collectiveData.collectiveName },
        { label: 'Date Created', value: `${agent.collectiveData.dayCreated}/${agent.collectiveData.monthCreated}/${agent.collectiveData.yearCreated}` },
        { label: 'Participants', value: agent.collectiveData.participants }
      ]);
    }
    ...
    // Download the PDF
    const fileName = `${agent.fullname || 'Agent'}_Details.pdf`;
    doc.save(fileName);
  };
  ...
  const getAvailableTabs = () => {
    ...
    if (agent.typeStatus?.business?.isComplete) {
      tabs.push({ key: 'legal', label: 'Legal Entity' });
    }
    if (agent.typeStatus?.collective?.isComplete) {
      tabs.push({ key: 'collective', label: 'Collective Group' });
    }
